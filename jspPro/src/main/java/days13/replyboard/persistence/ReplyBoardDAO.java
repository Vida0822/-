...
public class ReplyBoardDAO implements IReplyBoard{
		...
		try {
			...
			if ( rs.next() ) {
				...
				do {
					...
					dto.setNewImg(  new Boolean(  rs.getString("new") )  );		// db에는 없지만 dto엔 추가한 속성 	
	...
	public int insert(Connection con, ReplyBoardDTO dto) throws SQLException {
		// 새글 insert  ,  답글 insert  확인 방법 ? 답글은 반드시 부모글 정보가 있다.
		// 새글 :    목록페이지에서 "글쓰기" 버튼을 클릭.
		// 답글 :    글상세보기 페이지에서  "답글" 버튼 클릭.  
		//    (   부모글num, 부모글ref,부모글step, 부모그depth )
		
		// 파라미터 ReplyBoardDTO dto
		// 새글  글번호(num) : 시퀀스 자동 추가. 
		// 답글  글번호(num) / 그룹 ref :  부모의 그룹(ref) 숨기놓고..
		PreparedStatement pstmt = null;
		int rowCount = 0;
	...
	public int updateRefStep(Connection con, int parentRef, int parentStep) throws SQLException {
		String sql = "update replyboard "
				+ " set step = step + 1 "
				+ " where ref=? and step > ?";   // 부모의 step 보다 큰 애들의 step을 1씩 증가 
		PreparedStatement pstmt = null;
		int rowCount = 0;
		try {
			pstmt = con.prepareStatement(sql);
