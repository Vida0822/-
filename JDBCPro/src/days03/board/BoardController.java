...
public class BoardController {
	...
	private void 메뉴처리() {
		...
		case 4: // 수정
			...
			break;
		case 5: // 삭제
			삭제하기(); // -> ?? service.deleteService( ?? ) -> ?? dao.delete( ?? ) + 삭제완료 문구 + 일시정지
			break;
		case 6: // 검색
			검색하기();
			break;
	...
	private void 수정하기() {
		System.out.print("> 수정할 게시글번호(seq)를 입력하세요 : ");
		...
		// 수정할 게시글 정보를 입력 : 제목, 내용, 이메일 (실제로 수정할 수 있는거) 
	...
	private void 검색하기() {
		...
		System.out.print("> 1.제목 2.내용 3.작성자 4.제목+내용 검색조건 입력 : ");
		searchCondition = this.scanner.nextInt();
		System.out.print("> 검색어 입력 : ");
		searchWord = this.scanner.next();
		
		System.out.print("> 현재 페이지(currentPage) 입력 : ");
		...
		System.out.println("--------------------------------------------------------------");
		System.out.printf("%s\t%-15s\t%s\t%-10s\t%s\n", 
				"글번호","글제목","글쓴이","작성일","조회수");
		System.out.println("--------------------------------------------------------------");
		if (list == null) {
			...
			while (ir.hasNext()) {
				...
				System.out.printf("%d\t%-15s\t%s\t%-10s\t%d\n",
						dto.getSeq(), 
						dto.getTitle(),
						dto.getWriter(),
						dto.getWritedate(),
		...
		String pagingBlock = this.service.pageService(searchCondition, searchWord, this.currentPage, this.numberPerPage, this.numberOfPageBlock);
	...
	private void 삭제하기() {
		System.out.print("> 삭제할 게시글번호(seq)를 입력하세요 : ");
		int seq = this.scanner.nextInt();
	...
	private void 상세보기() {
		System.out.print("> 게시글번호(seq)를 입력하세요 : ");
		int seq = this.scanner.nextInt();
		BoardDTO dto = this.service.viewService(seq);
		// 해당 게시글 정보 출력( View 담당)
		System.out.println("\tㄱ. 글번호 : " + seq );
	...
	private void 새글쓰기() {
		...
		BoardDTO dto = new BoardDTO(0, writer, pwd, email, title, null, 0, tag, content); // 번호, 작성일, 조회수 제외
