	      	...
	      	<tr>
		        <td colspan="5">
			          <button id="search" class="search" type="button" >search</button>
			           <!-- form에서 button은 기본이 submit기능이라 type="button"을 해주면 submit 안됨  -->
	...
	// 확인버튼 클릭  ajax 추가 + 테이블 추가
	   $("#add-modal #add-dept").on("click", function (){
		     ...
		     deptService.add(dept,function(result){
		    	 addModal.css("display", "none") ; // 모달창 닫기 
				 if(result === 'SUCCESS'){ // db 작업 성공 (백앤드 작업은 끝, 이제 프론트에서 표시만 해주면 됨)
				 // 반복문으로 출력해주던걸 그냥 바로 tr,td 태그 수기로 추가 
					let tr = $(`
						<tr>
				 	...
				 	$(tr)
				 		...
				 		.find("span.delete")
				 			// .on("click", function(event) { // 이벤트를 '장착' 했다고 생각 
				 			.on("click", function() { // 이벤트를 '장착' 했다고 생각 
				 				if (confirm("정말 삭제할까요?")) {
				 					let deptno = $(this).data("deptno"); // 삭제 클릭한 부서번호 가져와서 
				 					var spanDelete = $(this) ;  
	   ...
	   // Edit - x버튼 
   $("#tbl-dept > tbody > tr > td > span").on("click", function() { // 로딩되어질 때 존재하는 객체들한테만 이벤트가 갈리기 때문에 새로 생성된 애들은 그 시점 이후라 이벤트가 적용되지 않는다
			if (confirm("정말 삭제할까요?")) {
				let deptno = $(this).data("deptno"); // 삭제 클릭한 부서번호 가져와서 
				deptService.remove(deptno, function(result) {
					if(result === 'SUCCESS') $(event.currentTarget).parents("tr").remove() ;  //  프론트단에서 삭제 
				}) ;  // remove
			}// if 
	}) ;  // 삭제 클릭 
