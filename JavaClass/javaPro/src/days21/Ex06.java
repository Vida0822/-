...
public class Ex06 {
	...
	public static void main(String[] args) {
		...
		// 생일 지났는지 / 지나지 않았는지 여부를 체크하는 코딩
		/*
		Date d = new Date();
		String rrn = "890211-1000000";
		int birth = Integer.parseInt(rrn.substring(2,6));
		int today = (d.getMonth()+1)*100 + d.getDate();
		System.out.println(birth < today ? "생일이 지났습니다" : "생일이 지나지 않았습니다");
		*/
		// 생일의 월,일
		// 오늘의 월,일
		String rrn = "890211-1";
		...
		System.out.println(d.equals(b_day)); // 밀리세컨드가 달라서 잘못된 결과가 나옴
		
		/*
		d.after(b_day); // true, false 참이면 생일 지난 것
		d.before(b_day); // true, false 참이면 생일 지나지 않은 것
		d.equals(b_day); // true, false 참이면 오늘 생일
		*/
		...
		*/
		...
		// b_month <= t_month && b_date < t_date  생일 지난 것
		// 그 외는 								  생일 안지난 것
		*/
